...
uint32_t TorrentConnectionv6::SendConnectRequest() {
    ...
    // magick number
    os.write(util::ToNetworkCharSequence((uint64_t) 0x41727101980ll).chars, sizeof(uint64_t));
    // action
    ...
    socket_.async_send(buffer.data(), [ep = end_point_](const ErrorCode &error, std::size_t bytes_transferred) {
        if (error) {
            Logger::get_instance()->Error(error.message() + " at endpoint "s + ep.address().to_string());
        }
        Logger::get_instance()->Info(std::to_string(bytes_transferred) + " bytes transferred to "s +
